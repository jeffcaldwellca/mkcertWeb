...
async function loadSystemStatus() {
    try {
        ...
        // Show notification if CA was auto-generated. caRoot is a filesystem
        // path returned by mkcert — escape before embedding into intentional HTML.
        if (status.autoGenerated) {
            showAlertHtml(
                ...
                '<em>Location:</em> ' + escapeHtml(status.caRoot || 'Default location'),
                'success'
            );
        }
...
function displayCertificates(certificates) {
    ...
    const html = certificates.map(cert => {
        ...
        // SECURITY: domains come from the parsed certificate (openssl x509 -text);
        // a malicious/uploaded cert can include arbitrary DNS-name SANs. Escape
        // before injecting into HTML.
        const rawDomains = cert.domains ? cert.domains.join(', ') : 'Unknown';
        let domainsDisplay;
        if (rawDomains.length > 100) {
            const truncated = rawDomains.substring(0, 97) + '...';
            domainsDisplay = `<span title="${escapeAttr(rawDomains)}">${escapeHtml(truncated)}</span>`;
        } else {
            domainsDisplay = escapeHtml(rawDomains);
        }
        ...
        // Pre-escape every server-supplied string we're about to interpolate.
        const safeName       = escapeHtml(cert.name);
        const safeCertFile   = escapeHtml(certFile || 'Unknown');
        const safeKeyFile    = keyFile ? escapeHtml(keyFile) : '<em>Missing</em>';
        const safeFolderDisp = escapeHtml(folderDisplay);
        ...
        return '<div class="certificate-card ' +
               ...
               '<i class="fas fa-' + (isRootCA ? 'shield-alt' : 'certificate') + '"></i> ' + safeName +
               ...
               '<div><strong>Location:</strong><br>' + safeFolderDisp + '</div>' +
               '<div><strong>Created:</strong><br>' + safeCreated + '</div>' +
               '<div class="' + expiryClass + '"><strong>Expiry:</strong><br>' + safeExpiry + '</div>' +
               '<div><strong>Certificate File:</strong><div class="file-name">' + safeCertFile + '</div></div>' +
               '<div><strong>Private Key File:</strong><div class="file-name">' + safeKeyFile + '</div></div>' +
               '<div><strong>File Size:</strong><br>' + safeSize + '</div>' +
...
// Alert system. By default messages are treated as plain text (escaped before
// rendering), because the vast majority of callers pass `error.message` or
// server-returned strings that may include user-controlled content. For the
// few cases where intentional markup is needed, call showAlertHtml().
function showAlert(message, type = 'info') {
    showAlertHtml(escapeHtml(message), type);
}

function showAlertHtml(htmlMessage, type = 'info') {
...
function displayFileList(files) {
    ...
    files.forEach(file => {
        ...
        // Build with DOM nodes so file.name (user-supplied) cannot be interpreted
        // as HTML — textContent escapes for us automatically.
        ...
        iconEl.className = `fas fa-${icon} file-icon`;
        ...
        info.append(iconEl, ' ', nameEl, ' ', sizeEl);

        const status = document.createElement('div');
        status.className = 'file-status';
        status.dataset.file = file.name;
        const clockIcon = document.createElement('i');
        clockIcon.className = 'fas fa-clock';
        const statusText = document.createElement('span');
        statusText.textContent = 'Pending';
        status.append(clockIcon, ' ', statusText);

        fileItem.append(info, status);
...
async function uploadFiles(files) {
    ...
    try {
        ...
        // FormData uploads can't go through apiRequest (which forces JSON
        // Content-Type), so we attach the CSRF token manually here.
        ...
        // Handle stale CSRF token (e.g. after a server restart): refresh + retry once.
...
function displayUploadResults(result) {
    ...
    if (result.incompletePairs && result.incompletePairs.length > 0) {
        // SECURITY: pair.certName and pair.hasFile come from the user's uploaded
        // filenames; the server's filename validator only rejects path-separator
        // and a few control chars, so HTML metacharacters can reach us. Escape.
        html += `
            <div class="upload-warning">
                ...
                <ul style="margin-top: 0.5rem; margin-left: 1rem;">
                    ${result.incompletePairs.map(pair =>
                        `<li>${escapeHtml(pair.certName)}: Missing ${escapeHtml(pair.missing)} (has: ${escapeHtml(pair.hasFile)})</li>`
                    ).join('')}
                </ul>
            </div>
        `;
...
async function loadEmailStatus() {
    try {
        ...
        if (!response.enabled) {
            ...
            // SMTP host/from/recipients come from settings.json / env vars;
            // operators control them, but they can still contain characters
            // that would corrupt the page if pasted carelessly. Escape.
            statusHtml = `
                ...
                <div style="margin-top: 0.8rem; font-size: 0.9rem;">
                    <div><strong>SMTP Host:</strong> ${escapeHtml(response.smtp.host)}</div>
                    <div><strong>Port:</strong> ${escapeHtml(String(response.smtp.port))} ${response.smtp.secure ? '(SSL)' : '(TLS)'}</div>
                    <div><strong>From:</strong> ${escapeHtml(response.from)}</div>
                    <div><strong>Recipients:</strong> ${response.to ? escapeHtml(response.to.join(', ')) : 'Not configured'}</div>
                </div>
            `;
        ...
        if (statusElement) {
            ...
            statusElement.innerHTML = `
                <div class="status-indicator error">
                    <i class="fas fa-exclamation-triangle"></i>
                    Failed to load email status: ${escapeHtml(error.message)}
                </div>
            `;
        }
    }
...
async function loadMonitoringStatus() {
    try {
        ...
        if (!response.enabled) {
            ...
            // response.schedule is a cron expression set in config; escape in
            // case the operator types HTML characters into it.
            statusHtml = `
                ...
                <div style="margin-top: 0.8rem; font-size: 0.9rem;">
                    <div><strong>Schedule:</strong> ${escapeHtml(response.schedule)}</div>
                    <div><strong>Warning Period:</strong> ${escapeHtml(String(response.warningDays))} days</div>
                    <div><strong>Critical Period:</strong> ${escapeHtml(String(response.criticalDays))} days</div>
        ...
        if (statusElement) {
            ...
            statusElement.innerHTML = `
                <div class="status-indicator error">
                    <i class="fas fa-exclamation-triangle"></i>
                    Failed to load monitoring status: ${escapeHtml(error.message)}
                </div>
            `;
        }
    }
...
async function loadExpiringCertificates() {
    try {
        ...
        response.certificates.forEach(cert => {
            ...
            // SECURITY: cert.path, cert.domains come from the server's parse of
            // the certificate file; a malicious cert can embed HTML in its SANs.
            // Escape everything before HTML interpolation.
            const safePath    = escapeHtml(cert.path);
            const safeDomains = cert.domains ? escapeHtml(cert.domains.join(', ')) : '';
            const safeExpires = escapeHtml(new Date(cert.expiry).toLocaleDateString());
            const days        = Number(cert.daysUntilExpiry);
            listHtml += `
                <div class="expiring-cert-item ${priorityClass}">
                    <div class="expiring-cert-header">
                        <div class="expiring-cert-path">${safePath}</div>
                        <div class="expiring-cert-days ${priorityClass}">
                            ${days} day${days !== 1 ? 's' : ''} remaining
                        </div>
                    </div>
                    <div class="expiring-cert-details">
                        <div><strong>Expires:</strong> ${safeExpires}</div>
                        ${safeDomains ? `<div class="expiring-cert-domains"><strong>Domains:</strong> ${safeDomains}</div>` : ''}
